        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                if not row:
                    raise HTTPException(
                        status_code=404, 
                        detail=f"Troquel '{troquel}' no encontrado en la tabla de medicamentos HIV"
                    )
                
                codigo_original, sustituye, codigo_sustituible = row
                
                # Convertir a entero por si la base devuelve string o float
                try:
                    is_sustituible = int(sustituye) == 1
                except (ValueError, TypeError):
                    is_sustituible = False

                if is_sustituible:
                    return SustitucionResponse(
                        troquel=troquel,
                        codigo_original=str(codigo_original),
                        es_sustituible=True,
                        mensaje=f"El medicamento es sustituible. Código original '{codigo_original}' se sustituye por '{codigo_sustituible}'",
                        codigo_sustituto=str(codigo_sustituible)
                    )
                else:
                    return SustitucionResponse(
                        troquel=troquel,
